...
%
\institute{Signal Power and Light, Inc. Cordova, AL 35550, USA
    \email{crabel@signalpowerandlight.com}\\
    \url{https://signalpowerandlight.com}}
%
\maketitle
%
\begin{abstract}
...
% The final model considers the known uncertainty of our previous estimations, providing an uncertainty adjusted metric for project evaluation.
...
\begin{figure}
    \includegraphics[width=\textwidth]{BTC Marginal Value.pdf}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Simultaneous Measurement of Value and Time
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% For this strategy, the initial $E_{in,i}$ of equation \ref{eq:12} would be 0 as the revenue would go to repay the loan.
...
% One of the failings of game theory and the concept of expected value is that uncertainty is not formally taken into account with the model.
...
%     F = E - T S. \label{eq:13}
% \end{equation}
% Where $F$ is the Helmholtz potential, $E$ is the value, $T$ is the economic temperature, and $S$ is the utility.
% The total value is one of any of the options, e.g. selecting the miner, site location, hodling bitcoin, selling bitcoin, leveraging bitcoin, fluctuations in the price of power, lack of availability of power etc.
% All of the model's assumptions, priors, are represented as statistical distributions.
% The model then samples the distributions and generates an estimate of the projects total value, $E_{total}$.
% The posterior distribution is the distribution of the $E_{total}$ from each sample.
...
% The economic temperature, marginal value of uncertainty can be taken directly as the value of capital expenditure for the project, $E_{expended}$.

% This allows us to rewrite equation \ref{eq:13},
% \begin{eqnarray}
%     F &=& E_{total} - E_{expended} S, \\
%     \frac{F}{E_{expended}} &=& \frac{\langle E_{total}\rangle}{E_{expended}} - S. \label{eq:14}
% \end{eqnarray}
% The selection criteria is to choose the option with the greatest Helmholtz potential per unit of value expended.
% It is important to note that the Value Return on Investment, VROI, is $\frac{\langle E_{total}\rangle}{E_{expended}}$, and that it is modified to take into account uncertainty.
% The temperature acts as a measure to discount the uncertainty of the project.
% In this way a project with a high value and high uncertainty, would be penalized much more than a project with a lower value and a correspondingly lower uncertainty.
